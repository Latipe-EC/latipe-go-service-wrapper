...
func (o orderCommandService) StoreUpdateOrderStatus(ctx context.Context, dto *store.StoreUpdateOrderStatusRequest) error {
	...
	case order.ORDER_PREPARED:
		...
		bodyContent = fmt.Sprintf("Xin chào, đơn hàng của bạn đã được cửa hàng xác nhận #[%v]", strings.ToUpper(orderDAO.OrderID))
	case order.ORDER_CANCEL_BY_STORE:
		...
		bodyContent = fmt.Sprintf("Xin lỗi đơn hàng #[%v] của bạn vừa bị hủy lý do là: %v ", strings.ToUpper(orderDAO.OrderID),
...
func (o orderCommandService) DeliveryUpdateOrderStatus(ctx context.Context, dto delivery.UpdateOrderStatusRequest) error {
	...
	case order.ORDER_DELIVERY:
		...
		bodyContent = fmt.Sprintf("Xin chào, đơn hàng [%v] của bạn đã được vận chuyển bởi %v ", strings.ToUpper(orderDAO.OrderID), orderDAO.Delivery.DeliveryName)

	case order.ORDER_CANCEL_USER_REJECT:
		orderDAO.Status = dto.Status
		if err := o.orderRepo.UpdateStatus(ctx, orderDAO.OrderID, dto.Status,
			fmt.Sprintf("Đơn hàng không thể giao thành công lý do: %v", dto.Message)); err != nil {
			return err
		}
		bodyContent = fmt.Sprintf("Xin chào, đơn hàng [%v] của bạn đã bị huỷ do %v ", strings.ToUpper(orderDAO.OrderID), dto.Message)
	...
	case order.ORDER_SHIPPING_FINISH:
		...
		bodyContent = fmt.Sprintf("Xin chào, đơn hàng [%v] của bạn đã giao thành công.", strings.ToUpper(orderDAO.OrderID))
...
func (o orderCommandService) UpdateOrderStatusByReplyMessage(ctx context.Context, dto *msgDTO.OrderStatusMessage) error {
	...
	if dto.Status == msgDTO.ORDER_EVENT_COMMIT_SUCCESS {
		bodyContent = fmt.Sprintf("Cửa hàng của bạn vừa có 1 đơn hàng chờ xác nhận từ hệ thống [%v]", strings.ToUpper(orderDAO.OrderID))
